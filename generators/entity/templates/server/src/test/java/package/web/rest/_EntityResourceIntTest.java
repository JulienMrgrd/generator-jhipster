...
import org.springframework.test.context.junit4.SpringRunner;<% if (reactive==='no') { %>
...
import java.util.List;<% if (reactive==='no') { %>
...
import static org.assertj.core.api.Assertions.assertThat;<% if (reactive === 'no') { %>
...
<%_ } %>
    ...
    public void setup() {
        ...
        <%_ } else { _%>
        this.webTestClient = WebTestClient.bindToController(labelResource)
//            .argumentResolvers(pageableArgumentResolver)
    ...
    public void initTest() {
        ...
        <%= entityInstance %>Repository.deleteAll()<% } if (reactive !== 'no') { %>.block()<% } %>;
    ...
    public void create<%= entityClass %>() throws Exception {
        int databaseSizeBeforeCreate = <%= entityInstance %>Repository.findAll()<% if (reactive !== 'no') { %>.collectList().block()<% } %>.size();

        // Create the <%= entityClass %>
        <%_ if (dto === 'mapstruct') { _%>
        <%= entityClass %>DTO <%= entityInstance %>DTO = <%= entityInstance %>Mapper.toDto(<%= entityInstance %>);
        <%_ } _%><%_ if (reactive === 'no') { %>
    ...
    public void check<%= fields[idx].fieldInJavaBeanMethod %>IsRequired() throws Exception {
        int databaseSizeBeforeTest = <%= entityInstance %>Repository.findAll()<% if (reactive !== 'no') { %>.collectList().block()<% } %>.size();
        // set the field null
...
<%  } } } %>
    ...
    public void update<%= entityClass %>() throws Exception {
        ...
        <%_ } else { _%>
        <%= entityInstance %>Repository.save<% if (databaseType === 'sql') { %>AndFlush<% } %>(<%= entityInstance %>)<% if (reactive !== 'no') { %>.block()<% } %>;<% if (searchEngine === 'elasticsearch') { %>
        <%= entityInstance %>SearchRepository.save(<%= entityInstance %>);<%_ } _%>
        <%_ } _%>

        int databaseSizeBeforeUpdate = <%= entityInstance %>Repository.findAll()<% if (reactive !== 'no') { %>.collectList().block()<% } %>.size();

        // Update the <%= entityInstance %>
        <%_ if (reactive === 'no') { _%>Optional<<%= entityClass %>> optionalUpdated<%= entityClass %> = <%= entityInstance %>Repository.<%_ if (databaseType !== 'cassandra') { _%>findById<%_ } else { _%>findOne<%_ }_%>(<%= entityInstance %>.getId());
    ...
    public void delete<%= entityClass %>() throws Exception {
        ...
        int databaseSizeBeforeDelete = <%= entityInstance %>Repository.findAll()<% if (reactive !== 'no') { %>.collectList().block()<% } %>.size();
